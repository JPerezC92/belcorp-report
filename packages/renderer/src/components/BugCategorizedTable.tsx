...
interface BugCategorizedRecord {
...
interface BugCategorizedTableProps {
	data: BugCategorizedRecord[];
}

const BugCategorizedTable: React.FC<BugCategorizedTableProps> = ({ data }) => {
			...
			<div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8 mt-6">
				<div className="text-center">
					<h3 className="text-lg font-semibold text-gray-900 mb-2">
						Bug Categorization Overview
					</h3>
					<p className="text-gray-500">
						No records found with "Error de codificación (Bug)" categorization
					</p>
				</div>
			</div>
		);
		...
		<div className="bg-white rounded-lg shadow-sm border border-gray-200 mt-6">
			...
			<div className="px-6 py-4 border-b border-gray-200">
				...
				<p className="text-sm text-gray-600 mt-1">
					Records categorized as "Error de codificación (Bug)" with corrective maintenance details
				</p>
			...
			<div className="px-6 py-3 bg-gray-50 border-t border-gray-200 text-sm text-gray-600">
				Showing {data.length} grouped bug record{data.length !== 1 ? "s" : ""}
			</div>
		</div>
	);
};

export default BugCategorizedTable;
